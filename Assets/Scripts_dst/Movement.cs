...
public class Movement : MonoBehaviour
{
    ...
    public Dictionary<Gamemodes, Sprite> GamemodeSprites;

    void Start()
    {
        rb = GetComponent<Rigidbody2D>();

        // Inicializa el diccionario con los sprites correspondientes
        GamemodeSprites = new Dictionary<Gamemodes, Sprite>
        {
            { Gamemodes.Cube, Resources.Load<Sprite>("Cube") },
            { Gamemodes.Ship, Resources.Load<Sprite>("Ship") },
            { Gamemodes.Ball, Resources.Load<Sprite>("Ball") },
            { Gamemodes.UFO, Resources.Load<Sprite>("UFO") },
            { Gamemodes.Wave, Resources.Load<Sprite>("Wave") },
            { Gamemodes.Robot, Resources.Load<Sprite>("Robot") },
            { Gamemodes.Spider, Resources.Load<Sprite>("Spider") }
        };
    }
    ...
    public void ChangeThroughPortal(Gamemodes Gamemode, Speeds Speed, int gravity, int State, float yPosPortal)
    {
        switch (State)
        {
            ...
            case 1:
                ...
                // Cambia el sprite del personaje según el Gamemode
                if (GamemodeSprites.ContainsKey(Gamemode))
                {
                    Sprite.GetComponent<SpriteRenderer>().sprite = GamemodeSprites[Gamemode];
                }
                break;
            case 2:
                Gravity = gravity;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        else if (collision is PolygonCollider2D)
        {
            ResetPlayer();
        }
    }
